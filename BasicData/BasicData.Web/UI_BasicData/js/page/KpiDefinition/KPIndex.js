...
$(document).ready(function () {
    ...
    //if ($("input[name=adminFlag]").get(0).checked == false) {
    //    var text = $("input[name=adminFlag]").get(0).value;
    //    $.messager.alert('提示', text);
    //} else {
    //    $.messager.alert('提示', 'afjal');
    //}
...
function LoadStandardType() {
    $.ajax({
        type: "POST",
        url: "KPIndex.aspx/GetStandardType",
...
function LoadConsumpation(StandardId)
{
    $.ajax({
        type: "POST",
        url: "KPIndex.aspx/GetConsumpationType",
        ...
        success: function (msg) {
            ...
            $('#cbox_Consumpation').combobox({
                ...
                onSelect: function (param) {
                    ...
                    //LoadIndex(m_Consumpation,m_KeyId)  
                    if (m_Consumpation == "综合能耗") {
                        $("#txt_StandardType").combobox({
                            textField: 'type',
                            valueField: 'value',
                            data: [{
                                type: '能源管理',
                            ...
                            onSelect: function (para) {
                                m_StandardType = para.value;
                            }
                        });
                    }
                    else if (m_Consumpation == "标准能耗") {
                        $("#txt_StandardType").combobox({                           
                            textField: 'type',
                            valueField:'value',
                            data:[{
...
function LoadIndex_Clinck() {
    ...
    var mKeyId = m_KeyId;
    var mStandardType = m_StandardType;
...
function CreateKPIndex() {       
    $("#dlgKPIndexDetailEditor").dialog('open');
    //$("input[name=adminFlag]").change(function () {
    //    var m_radValue = $("input[name=adminFlag]").value;
    //    if (m_radValue == "0") {
    //        $("#KPIndexOrganization").combobox({ disabled: true });
    //    } else if (m_radValue == "1") { $("#KPIndexOrganization").combobox({ disabled: false }); }
    //});

    //初始化//
    LoadKPIndexOrganizationData("first"); 
    $("input[name=adminFlag]").get(0).checked = false;
    $("input[name=adminFlag]").get(1).checked = false;
    $("#KPIndexOrganization").combotree({ disabled: true });
}
function radioYes() {
    $("#KPIndexOrganization").combotree({ disabled: false });
}
function radioNo() {
    $("#KPIndexOrganization").combotree({ disabled: true });
    $.ajax({
        type: "POST",
        url: "KPIndex.aspx/GetObjectNameList",
        data:"",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (msg) {
...
function LoadKPIndexOrganizationData(myLoadType) {
    $.ajax({
        type: "POST",
        url: "KPIndex.aspx/GetKPIndexOrganizationData",
        data: "",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (msg) {
            ...
            if (myLoadType == 'first') {
                ...
                $('#KPIndexOrganization').combotree('loadData', m_MsgData);
...
function LoadNameList(OrganizationId) {
    $.ajax({
        type: "POST",
        url: "KPIndex.aspx/GetNameListData",
...
function KPIndexDetailSave() {
   ...
   if (txt_ObjName == "" || txt_StandardLevel == "" || txt_EnergyType == ""|| txt_Name == "" || txt_Creator == "") {
       ...
       $.ajax({
           type: "POST",
           url: "KPIndex.aspx/AddKPIndex",
           data: queryData,
           contentType: "application/json; charset=utf-8",
           dataType: "json",
           success: function (msg) {
               $("#dlgKPIndexDetailEditor").dialog('close');
...
{
    var queryUrl = 'KPIndex.aspx/DeleteKPIndexData';
